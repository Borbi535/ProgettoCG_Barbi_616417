...
struct carousel_loader {
	...
	static int load(const char* svgFile, const char* terrain_image, race& r) {
		...
		if (data == NULL)
		{
			std::cout << stbi_failure_reason() << std::endl << terrain_image << std::endl;
			perror("Error at load in carousel_loader.h");
		}
		...
		for (NSVGshape* shape = image->shapes; shape != NULL; shape = shape->next) 
		{
			...
			else if (std::string(shape->id).find("lamp") != std::string::npos) 
				push_stick_object(shape->paths, 2.f, r._lamps);
			else if (std::string(shape->id).find("cameraman") != std::string::npos)
			{
				...
				float radius = (float) atof(rad.c_str()); // questo sarebbe 15 dovrebbe stare al posto di 15.f qui sotto
				push_cameraman(shape->paths, radius, r._cameramen);  // push_cameraman(shape->paths, 15.f, r._cameramen);
			}
			else if (std::string(shape->id).find("track") != std::string::npos)
			{
				int ip = 0;
			...
			else if (std::string(shape->id).find("carpath") != std::string::npos)
			{
				int ip = 0;
				std::vector<glm::vec3> samples_pos, samples_tan;
