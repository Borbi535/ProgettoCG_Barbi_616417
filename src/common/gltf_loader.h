...
struct gltf_loader {
			...
			for (size_t i = 0; i < mesh.primitives.size(); i++)
			{
					...
					if (attr_index != -1) 
					{
						...
						switch (accessor.componentType) 
						{
							case TINYGLTF_PARAMETER_TYPE_FLOAT:
								r.add_vertex_attribute<float>((float*)&model.buffers[buffer].data[bufferviewOffset + accessor.byteOffset], n_comp * n_vert, attr_index, n_comp);
								break;
							case TINYGLTF_PARAMETER_TYPE_BYTE:
								r.add_vertex_attribute<char>((char*)&model.buffers[buffer].data[bufferviewOffset + accessor.byteOffset], n_comp * n_vert, attr_index, n_comp);
								break;
							case TINYGLTF_PARAMETER_TYPE_UNSIGNED_BYTE:
								r.add_vertex_attribute<unsigned char>((unsigned char*)&model.buffers[buffer].data[bufferviewOffset + accessor.byteOffset], n_comp * n_vert, attr_index, n_comp);
								break;
						}
				...
				switch (indexAccessor.componentType) 
				{
					case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:
						r.add_indices<unsigned char>((unsigned char*)&model.buffers[buffer].data[bufferviewOffset + indexAccessor.byteOffset],
													 (unsigned int)indexAccessor.count,
													 GL_TRIANGLES); 
						break;

					case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT:
						r.add_indices<unsigned short>((unsigned short*)&model.buffers[buffer].data[bufferviewOffset + indexAccessor.byteOffset],
													  (unsigned int) indexAccessor.count,
													  GL_TRIANGLES);
						break;

					case TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT:
						r.add_indices<unsigned int>((unsigned int*)&model.buffers[buffer].data[bufferviewOffset + indexAccessor.byteOffset],
													(unsigned int) indexAccessor.count,
													GL_TRIANGLES); 
						break;
				}
	...
	bool create_renderable(std::vector<renderable>& _renderable, box3& bbox) 
	{
		...
		// load texture
		for (unsigned int it = 0; it < model.textures.size(); ++it/*it++*/)
		{
			...
			// decode the image
			tinygltf::BufferView bufferview = model.bufferViews[image.bufferView]; // Expression: vector subscript out of range
